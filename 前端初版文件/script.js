...
function initChapterCards() {
    ...
    editBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            const chapterTitle = this.closest('.chapter-card').querySelector('.chapter-title.zh').textContent;
    ...
    prepareBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            const chapterTitle = this.closest('.chapter-card').querySelector('.chapter-title.zh').textContent;
            showNotification(`正在进入备课: ${chapterTitle}`, 'info');
            
            // 模拟导航到备课界面
            const aiPreNav = document.querySelector('.nav-item[data-section="ai-pre"]');
            aiPreNav.click();
        });
    });
    ...
    teachBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            const chapterTitle = this.closest('.chapter-card').querySelector('.chapter-title.zh').textContent;
            showNotification(`正在进入课堂: ${chapterTitle}`, 'success');
            
            // 模拟导航到课中界面
            const aiInNav = document.querySelector('.nav-item[data-section="ai-in"]');
            aiInNav.click();
        });
    });
...
function initChapterSlider() {
    ...
    if (!container || !prevBtn || !nextBtn || chapterCards.length === 0) return;
    
    // 计算每次滚动的距离（一个卡片的宽度加上间距）
    const calculateScrollDistance = () => {
        const cardWidth = chapterCards[0].offsetWidth;
        const gap = 24; // 与CSS中的gap值保持一致
    ...
    const updateButtonState = () => {
        ...
        prevBtn.style.cursor = scrollPosition <= 0 ? 'default' : 'pointer';
        
        // 如果已经滚动到最右侧，禁用下一个按钮
        const maxScroll = container.scrollWidth - container.clientWidth;
        nextBtn.style.opacity = scrollPosition >= maxScroll ? '0.5' : '1';
        nextBtn.style.cursor = scrollPosition >= maxScroll ? 'default' : 'pointer';
    };
    ...
    prevBtn.addEventListener('click', () => {
        ...
        scrollPosition = Math.max(0, scrollPosition - scrollDistance);
        container.scrollTo({
            left: scrollPosition,
            behavior: 'smooth'
        });
    ...
    nextBtn.addEventListener('click', () => {
        const scrollDistance = calculateScrollDistance();
        const maxScroll = container.scrollWidth - container.clientWidth;
        scrollPosition = Math.min(maxScroll, scrollPosition + scrollDistance);
        container.scrollTo({
            left: scrollPosition,
            behavior: 'smooth'
        });
    ...
    // 监听容器滚动事件，更新scrollPosition变量
    container.addEventListener('scroll', () => {
        scrollPosition = container.scrollLeft;
        updateButtonState();
    });
    
    // 监听窗口大小变化，重新计算滚动位置和按钮状态
    window.addEventListener('resize', () => {
        // 确保scrollPosition不超过最大滚动距离
        const maxScroll = container.scrollWidth - container.clientWidth;
